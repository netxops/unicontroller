...
func InitializeApp(cfg *config.Config, configPath string, logger *zap.Logger) (*App, error) {
	...
	systemdAdapter := provideSystemdAdapter(cfg)
	processManager := provideProcessManager(cfg, logger)
	serviceManager := service.NewServiceManager(serviceRegistry, systemdAdapter, processManager)
	configManager := service.NewConfigManager(serviceRegistry, logger)
	logManager := provideLogManager(cfg, logger)
	packageService := api.NewPackageService(serviceManager, configManager, logManager, logger)
	commandService := api.NewCommandService(logger)
	healthChecker := provideHealthChecker()
	autoRecovery := provideAutoRecovery(serviceManager, logger)
	coordinator := provideHealthCoordinator(healthChecker, autoRecovery, serviceManager, serviceRegistry, logger)
	healthService := provideHealthService(healthChecker, coordinator, serviceManager, logger)
	metricsCollector := provideMetricsCollector(cfg, serviceRegistry, logger)
	applicationMetricsCollector := provideApplicationMetricsCollector(cfg, logger)
	metricsService := provideMetricsService(metricsCollector, applicationMetricsCollector, serviceManager, logger)
	fileService := provideFileService(cfg, logger)
	fileGRPCService := provideFileGRPCService(fileService, logger)
	serverMetricsCollector := provideServerMetricsCollector(metricsCollector, logger)
	strategyManager := provideStrategyManager(cfg, logger)
	strategySyncer := provideStrategySyncer(cfg, strategyManager, metricsCollector, logger)
	server, err := provideServer(serverConfig, logger, packageService, commandService, healthService, metricsService, fileGRPCService, serverMetricsCollector, strategySyncer, cfg)
	if err != nil {
		return nil, err
	}
	registry, err := provideRegistry(cfg, logger)
	if err != nil {
		return nil, err
	}
	registryManager := provideRegistryManager(cfg, registry, serviceManager, logger, serverConfig)
	serviceDiscovery := provideServiceDiscovery(cfg, serviceRegistry, logger)
	metricsReporter := provideMetricsReporter(cfg, metricsCollector, logger)
	configReloader := provideConfigReloader(cfg, configPath, registryManager, serviceDiscovery, metricsCollector, metricsReporter, logManager, logger)
	app := NewApp(server, registryManager, serviceDiscovery, coordinator, metricsCollector, metricsReporter, configReloader, strategySyncer)
	return app, nil
}
...
func provideMetricsCollector(cfg *config.Config, registry *service.ServiceRegistry, logger *zap.Logger) metrics.MetricsCollector {
	logger.Info("Providing metrics collector", zap.Bool("metrics_enabled", cfg.Metrics.Enabled), zap.Bool("enhanced_enabled", cfg.Metrics.EnhancedEnabled), zap.Bool("collect_system", cfg.Metrics.CollectSystem), zap.Bool("collect_service", cfg.Metrics.CollectService))
	...
	metricsConfig := metrics.CollectorConfig{
		...
		EnhancedEnabled:  cfg.Metrics.EnhancedEnabled,
	...
	if cfg.Metrics.EnhancedEnabled {
	...
	logger.Info("Creating metrics collector with config", zap.Bool("enhanced_enabled", metricsConfig.EnhancedEnabled), zap.Bool("history_enabled", metricsConfig.HistoryEnabled), zap.Int("history_max_points", metricsConfig.HistoryMaxPoints), zap.Bool("enhanced_only_core", enhancedConfig.OnlyCore), zap.Int("enhanced_enabled_count", len(enhancedConfig.EnabledCollectors)), zap.Int("enhanced_exclude_count", len(enhancedConfig.ExcludeCollectors)))
...
func provideRegistry(cfg *config.Config, logger *zap.Logger) (registry.Registry, error) {
	...
	logger.Info("Creating etcd registry from config", zap.Strings("endpoints", cfg.Registry.EtcdEndpoints), zap.String("prefix", cfg.Registry.EtcdPrefix), zap.Bool("has_auth", hasAuth), zap.String("username", cfg.Registry.EtcdUsername))
...
func provideServiceDiscovery(
	cfg *config.Config, registry2 *service.ServiceRegistry,
	logger *zap.Logger,
	...
	return service.NewServiceDiscovery(
		cfg.Agent.Workspace,
		cfg.Discovery.Interval, registry2, logger,
	)
}
...
func provideHealthCoordinator(
	...
	serviceManager service.ServiceManager, registry2 *service.ServiceRegistry,
	logger *zap.Logger,
) *health.Coordinator {
	return health.NewCoordinator(
		healthChecker,
		autoRecovery,
		serviceManager, registry2, logger,
	)
}
...
func provideServerMetricsCollector(metricsCollector metrics.MetricsCollector, logger *zap.Logger) server.MetricsCollector {
	...
	if enhanced := adapter.GetEnhancedCollector(); enhanced != nil {
		logger.Info("Server metrics collector adapter initialized with enhanced collector", zap.String("enhanced_type", fmt.Sprintf("%T", enhanced)))
	} else {
		logger.Warn("Server metrics collector adapter initialized but enhanced collector is nil", zap.String("collector_type", fmt.Sprintf("%T", metricsCollector)))
	}
...
func provideApplicationMetricsCollector(cfg *config.Config, logger *zap.Logger) *metrics.ApplicationMetricsCollector {
