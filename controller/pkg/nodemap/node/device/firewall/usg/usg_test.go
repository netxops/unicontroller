...
func TestFindPolicyByIntent(t *testing.T) {
			...
			policySet: []*Policy{
				...
				},
				{
					...
					policyEntry: &policy.PolicyEntry{
						...
						PolicyEntryService: service.NewServiceMust("tcp:443"),
					},
					action: firewall.POLICY_PERMIT,
				},
				{
		...
		},
		{
			...
			intent: &policy.Intent{
				PolicyEntry: policy.PolicyEntry{
					PolicyEntrySrc:     mustNetworkGroup("192.168.1.0/24"),
					PolicyEntryDst:     mustNetworkGroup("10.0.0.0/24"),
					PolicyEntryService: service.NewServiceMust("udp:53"),
				},
			},
			fromZone:     "trust",
			toZone:       "untrust",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := common.FindPolicyByIntent(usg, tt.intent, tt.fromZone, tt.toZone, tt.config)
			...
			for i, policy := range result {
				assert.Contains(t, tt.expectedName, policy.Name(), "Policy name %s should be in expected Names", policy.Name())
				if i < len(tt.expectedName) {
					assert.Equal(t, tt.expectedName[i], policy.Name(), "Policy name should match expected Name")
				}
			}
		})
	}
