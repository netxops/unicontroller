...
func FortigateParseProtocol(s string) (int, error) {
	...
	s1 := strings.ToLower(s)
	v, ok := Fortigate_ProtocolToNum[s1]
	if !ok {
...
func FortigatePortParse(port string, protocol string) (int, error) {
	p, err := FortigateParseProtocol(protocol)
	if err != nil {
		return -1, err
	}
	if p == 6 {
		return FortigateTcpPortParse(port)
	} else if p == 17 {
		return FortigateUdpPortParse(port)
	} else if p == 256 {
		return FortigateTcpUdpPortParse(port)
	}
