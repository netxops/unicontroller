...
func StdConfig(name string) *Config {
	...
	return RawConfig("jupite.registry.custome_etcdv3" + name)
}
...
func RawConfig(key string) *Config {
	...
	if err := conf.UnmarshalKey(key, &config); err != nil {
		xlog.Jupiter().Panic("unmarshal key", xlog.FieldMod("registry.custome_etcd"), xlog.FieldErrKind(ecode.ErrKindUnmarshalConfigErr), xlog.FieldErr(err), xlog.String("key", key), xlog.Any("config", config))
	}
	// 解析嵌套配置
