...
func PasswordAuth(pass string) Option {
	return func(s *SSHClient) {
		s.AuthMethod = append(s.AuthMethod, ssh.Password(pass))
	}
}

func KeyAuth(privateKey string) Option {
	return func(s *SSHClient) {
...
func (c *SSHClient) Options(opt Option) {
...
func (c *SSHClient) ConnectWith(host string, dialer SSHDialFunc) error {
	...
	config.Auth = append(config.Auth, c.AuthMethod...)
...
func (c *SSHClient) Run(task *Task) error {
	...
	fmt.Println("start remote command: ", c.env, task.Run)
