...
func (a Usg) Process(remote *structs.L2DeviceRemoteInfo, taskConfig structs.L2NodemapTaskConfigInterface, options ...interface{}) (result *clitask.Table, err error) {
...
func (a Usg) config(remote *structs.L2DeviceRemoteInfo, taskConfig structs.L2NodemapTaskConfigInterface, options ...interface{}) (*clitask.Table, error) {
	cmds := []string{}
	...
	if len(cmds) == 0 {
		return nil, errors.New("usg commands not received, command is empty")
	}
	return usgTerminalConfig(remote, taskConfig, cmds)
}

func usgTerminalConfig(remote *structs.L2DeviceRemoteInfo, taskConfig structs.L2NodemapTaskConfigInterface, params []string) (result *clitask.Table, err error) {
	...
	if err != nil {
		err = errors.New("usg config result trans err")
		return
	}
