...
func (a Sangfor) Process(remote *structs.L2DeviceRemoteInfo, taskConfig structs.L2NodemapTaskConfigInterface, options ...interface{}) (result *clitask.Table, err error) {
...
func (a Sangfor) config(remote *structs.L2DeviceRemoteInfo, taskConfig structs.L2NodemapTaskConfigInterface, options ...interface{}) (*clitask.Table, error) {
	cmds := []string{}
	...
	if len(cmds) == 0 {
		return nil, errors.New("sangfor commands not received, command is empty")
	}
	return sangforTerminalConfig(remote, taskConfig, cmds)
}

func sangforTerminalConfig(remote *structs.L2DeviceRemoteInfo, taskConfig structs.L2NodemapTaskConfigInterface, params []string) (result *clitask.Table, err error) {
	...
	exec := terminal.NewExecute(terminalmode.CONFIG, terminalmode.Sangfor, base)

	fmt.Println("sangfor method params======>", params)
	var cmds []*terminalmode.Command
	for index, op := range params {
		key := fmt.Sprintf("%s_%d", op, index+1)
		cmd := terminalmode.NewCommand(op, "", 1, key, "")
		exec.AddCommand(cmd)
	...
	if err != nil {
		err = errors.New("sangfor config result trans err")
		return
	}
